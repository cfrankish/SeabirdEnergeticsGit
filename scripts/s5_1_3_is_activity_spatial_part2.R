...
projection_NA<-"+proj=laea +x_0=0 +y_0=0 +lon_0=-9 +lat_0=61"
#rast.trans<-raster::projectRaster(rast.gr, crs=projection_NA, res=gridSize, method="bilinear")
...
if (nrow(rastDF_sub)<1) {
# kittiwakes in PAcific
next
}
...
# Now we save these birds in a population-level one
griddedBirds_pop<-rbind(griddedBirds_pop, griddedBirds)

} # End of population loop

# Add coords.x1 and coords.x2 #
griddedBirds_pop$coords.x1<-griddedBirds_pop$x
griddedBirds_pop$coords.x2<-griddedBirds_pop$y

###### Build a GAM to predict behavior accross whole North Atlantic ######

# Make a dataset to predicton (Per's pop maps)#
xyCoords<-as.data.frame(rast.coarse, xy=TRUE)
xyCoords<-xyCoords %>%
rename(coords.x1=x, coords.x2=y) %>%
dplyr::mutate(individ_id=meanBeh$individ_id[1]) %>%
dplyr::mutate(colony=meanBeh$colony[1])

# Define response variable
responseVar<-paste0("prop", Behaviour) 
responseVar2<-paste0(Behaviour) 

# Helper: try a sequence of k values
fit_gam_retry_k <- function(formula_fun, data, k_values = c(100, 75, 50, 25, 10)) {
  ...
  for (k in k_values) {
    ...
    fit <- tryCatch(
      ...
      error = function(e) {
        last_error <<- e
        message("Failed at k = ", k, ": ", conditionMessage(e))
        NULL
      }
    )
    
    if (!is.null(fit)) {
      message("Success with k = ", k)
      return(fit)
    }
  }
  
  stop("All k values failed. Last error: ", conditionMessage(last_error))
}

# Define formulas
gam_formula1 <- propBeh ~ s(individ_id, bs = "re")

gam_formula2_fun <- function(k) {
  as.formula(
    paste0(
      "propBeh ~ s(coords.x1, coords.x2, bs = 'gp', k = ", k, 
      ") + s(individ_id, bs = 're')"
    )
  )
}

gam_formula3_fun <- function(k) {
  as.formula(
    paste0(
      "propBeh ~ s(coords.x1, coords.x2, bs = 'gp', k = ", k,
      ", by = colony) + colony + s(individ_id, bs = 're')"
    )
  )
}
...
griddedBirds_pop$individ_id<-factor(griddedBirds_pop$individ_id)
griddedBirds_pop$colony<-factor(griddedBirds_pop$colony)
...
gam1 <- bam(
  gam_formula1,
  family = gaussian(),
  data = griddedBirds_pop,
  method = "REML"
)

gam2 <- fit_gam_retry_k(
  formula_fun = gam_formula2_fun,
  data = griddedBirds_pop,
  k_values = c(30, 20, 10)
)

gam3 <- fit_gam_retry_k(
  formula_fun = gam_formula3_fun,
  data = griddedBirds_pop,
  k_values = c(30, 20, 10)
)

endGam <- Sys.time()
print(paste0("Gam took: ", endGam - startGam))
...
griddedBirds_pop$AIC1<-AICModels$AIC[1]
griddedBirds_pop$AIC2<-AICModels$AIC[2]
griddedBirds_pop$AIC3<-AICModels$AIC[3]
griddedBirds_pop$dev1<-summary(gam1)$dev.expl
griddedBirds_pop$dev2<-summary(gam2)$dev.expl
griddedBirds_pop$dev3<-summary(gam3)$dev.expl

# Save these results
predictionSummaries<-rbind(predictionSummaries, griddedBirds_pop)
...
xyCoords<-xyCoords %>%
rename(coords.x1=x, coords.x2=y) %>%
dplyr::mutate(individ_id=griddedBirds_pop$individ_id[1]) %>%
dplyr::mutate(colony=griddedBirds_pop$colony[1])
...
statPoints<-xyCoords
predictions_temporal <- rasterize(statPoints, rast.coarse, field="predictions", fun=mean)
...
coordinates(xyCoords) <- ~ coords.x1 + coords.x2
statPoints<-xyCoords
predictions_space <- rasterize(statPoints, rast.coarse, field="predictions", fun=mean)
...
coloniesAll<-unique(griddedBirds_pop$colony)
#coloniesRandom<-sample(coloniesAll, 3, replace=FALSE)

for (col in 1:length(coloniesAll)) {

xyCoords<-as.data.frame(rast.coarse, xy=TRUE)
xyCoords<-xyCoords %>%
rename(coords.x1=x, coords.x2=y) %>%
dplyr::mutate(individ_id=griddedBirds_pop$individ_id[1]) %>%
dplyr::mutate(colony=coloniesAll[col])

Predictions_Outputs <- predict(gam3, newdata=xyCoords, exclude = c("s(individ_id)"), type="response")
xyCoords$predictions<-Predictions_Outputs
coordinates(xyCoords) <- ~ coords.x1 + coords.x2
statPoints<-xyCoords
predictions_colony <- rasterize(statPoints, rast.coarse, field="predictions", fun=mean)
...
names(colonyMapsStack)<-coloniesAll
...
# Summarize gridded information #
xyCoords<-as.data.frame(rast.coarse, xy=TRUE)
xyCoords$index<-1:nrow(xyCoords)

griddedSum<-predictionSummaries %>%
ungroup() %>%
dplyr::group_by(species, colony, index) %>%
dplyr::summarise(propBeh_mean=mean(propBeh, na.rm=TRUE), birdsTot=sum(birds, na.rm=TRUE)) %>%
ungroup() %>%
dplyr::group_by(species, index) %>%
dplyr::summarise(propBeh_mean=mean(propBeh_mean), popsTot=n_distinct(colony)) %>%
dplyr::left_join(xyCoords, by=c("index"))

griddedSum_sub<-subset(griddedSum, popsTot>0)
...
# Change values in predictions from 0-1
predictionsAllGam_model2_df$layer<-ifelse(predictionsAllGam_model2_df$layer<0, 0, predictionsAllGam_model2_df$layer)
predictionsAllGam_model2_df$layer<-ifelse(predictionsAllGam_model2_df$layer>1, 1, predictionsAllGam_model2_df$layer)
...
geom_sf(data=coast) +
coord_sf(crs=projection_84, xlim=c(min(griddedSum_sub$x), max(griddedSum_sub$x)), ylim=c(min(griddedSum_sub$y), max(griddedSum_sub$y))) +
...
plot2<-ggplot() +
#geom_tile(data=predictionsAllGam_model2_df, aes(x=x, y=y, fill=layer)) +
geom_tile(data=griddedSum, aes(x=x, y=y, fill=propBeh_mean)) +
geom_sf(data=world, color = "#E5E5E5", fill = "#E5E5E5") +
geom_sf(data=coast) +
scale_fill_gradientn(
  colours = c(
...
pdf(paste0("results/", input_file1, "_", Behaviour, "_", monthChoice, "v2.pdf"))
...
restacked <- lapply(seq_len(nlayers(predictionsAllGam_model3[[1]])), function(i) {
  stack(lapply(predictionsAllGam_model3, raster::subset, i))
})

# Pick three at random to make plots of 
layerNo<-length(restacked)
colonyRandom<-sample(1:layerNo, 3, replace=FALSE)

for (i in 1:length(colonyRandom)) {

colonySub<-colonyRandom[i] # Random colony i
restacked_sub<-restacked[[colonySub]] # Select relevant colony
restacked_sub_mean<-overlay(restacked_sub, fun="mean") # Make an average accross iterations
predictionsAllGam_model3_df<-as.data.frame(restacked_sub_mean, xy=TRUE) # Turn into data frame
predictionsAllGam_model3_df$predictions<-predictionsAllGam_model3_df[,3]
predictionSummaries$Beh<-predictionSummaries[[responseVar2]] # Add a column with the behaviour we are modelling
colonyNameMatch<-gsub("\\.", " ", names(restacked_sub)) # Extract colony name from the raster so I can subset the locations
colonyNameMatch<-unique(gsub("[0-9]", "", colonyNameMatch)) # remove layer numbers
colonyNameMatch<-trimws(colonyNameMatch)

print(paste0("Plotting " ,colonyNameMatch))
...
ungroup() %>%
dplyr::group_by(colony, index) %>%
dplyr::summarise(propBeh_mean=mean(propBeh, na.rm=TRUE), birdsTot=sum(birds, na.rm=TRUE)) %>%
dplyr::left_join(xyCoords, by=c("index")) %>%
droplevels()

griddedSum_sub<-subset(griddedSum, birdsTot>0)
...
maxRaw<-max(griddedSum_sub$propBeh_mean)
maxPred<-max(predictionsAllGam_model3_df$predictions)
maxPlot<-ifelse(maxRaw>maxPred, maxRaw, maxPred)
maxPlot<-ifelse(maxPlot>1, 1, maxPlot)

minRaw<-min(griddedSum_sub$propBeh_mean)
...
speciesPlot<-ggplot() +
geom_tile(data=predictionsAllGam_model2_df, aes(x=x, y=y, fill=layer)) +
geom_sf(data = hull_84, fill = NA, color = "red", linewidth = 1) +
geom_sf(data=world, color = "#E5E5E5", fill = "#E5E5E5") +
geom_sf(data=coast) +
coord_sf(crs=projection_84, xlim=c(min(griddedSum_sub$x), max(griddedSum_sub$x)), ylim=c(min(griddedSum_sub$y), max(griddedSum_sub$y))) +
scale_fill_gradientn(
...
# Change values in predictions from 0-1
predictionsAllGam_model3_df$predictions<-ifelse(predictionsAllGam_model3_df$predictions<0, 0, predictionsAllGam_model3_df$predictions)
predictionsAllGam_model3_df$predictions<-ifelse(predictionsAllGam_model3_df$predictions>1, 1, predictionsAllGam_model3_df$predictions)
