...
input_file1 <- args[1] # This will read in a species-specific density map - input_file1<-"data/popdata_raw/SEATRACK_Abundance_Model_Alle_alle_Ver_3_1.nc"
...
# Here we determine number of populations total. We loop through these & save individual csv files
populations<-subset(meta, modelcolonies == modelColonies[i]) # subset to model colony i
populations_unique<-unique(populations$colonies) # make sure colonies are unique

for (k in 1:length(populations_unique)) {

print(paste0("Saving for pop ", k, "/", length(populations_unique)))

# Add name of colony
meanActMonthly$colony<-populations_unique[k]
meanActDaily$colony<-populations_unique[k]
...
startName<-substr(populations_unique[k], 1, 7)

if (startName =="Rodolph") {
popSave<-"rodolphisland"
} else {
popSave<-gsub("_", "", populations_unique[k])
}
