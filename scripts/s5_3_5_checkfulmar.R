...
#print("act")
energyFiles<-speciesFiles[grep("energyMap_v2", speciesFiles)] # extract energy files
#print("energy")
...
# Make a plot
energyPlot<-energy_j_i %>%
dplyr::select(x, y, energyPopkJ_mean) %>%
dplyr::rename(value=energyPopkJ_mean) %>%
dplyr::mutate(metric="Energy_demand") 

plots<-ggplot() + 
geom_tile(data=energyPlot, aes(x=x, y=y, fill=value)) +
scale_fill_gradientn('scaled 0-1', colors=c("#364B9A", "#4A7BB7","#6EA6CD", "#98CAE1", "#C2E4EF", "#EAECCC", "#FEDA8B", "#FDB366", "#F67E4B", "#DD3D2D", "#A50026")) +
ggtitle(paste0(energy_j$modelcolony[1]))
...
energySpeciesCoords$DEEkJ_mean<-energySpeciesCoords$DEEkJ_mean/length(energyFiles)
...
pdf(paste0("/cluster/projects/nn11080k/cfrank93/cbirdEnergy/results/figures/speciesMaps/energy/", energySpeciesMonth$species[1], "_energy_v2.pdf"), width=10, height=7)
