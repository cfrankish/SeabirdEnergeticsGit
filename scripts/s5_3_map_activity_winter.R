...
# Calculate total time in different behaviours per period & subset to 4 winter months #
totalTime<-actBudgets %>%
ungroup() %>%
dplyr::filter(month %in% c(10, 11, 12, 1)) %>%
dplyr::group_by(individ_id) %>%
dplyr::mutate(months=n_distinct(month)) %>%
dplyr::filter(months==4) %>%
ungroup() %>%
...
# Project coordinates
actBudgetsSubset<-actBudgets %>%
ungroup() %>%
dplyr::filter(month %in% c(10, 11, 12, 1)) %>%
dplyr::group_by(individ_id) %>%
dplyr::mutate(months=n_distinct(month)) %>%
dplyr::filter(months==4)
coordinates(actBudgetsSubset)<-~meanLon + mean.lat
proj4string(actBudgetsSubset)<-projection_84
actMonthTrans<-data.frame(spTransform(actBudgetsSubset, projection_NA))

# Determine total birds accross month
totBirds<-n_distinct(totalTime$individ_id)
...
gridMonth$colony<-actMonthTrans$colony[1]
gridMonth$colony_og<-actBudgets$og_colony[1]
#gridMonth$timeTotal<-gridMonth$timeTotal/mean(totalTime$totHrs)
#gridMonth$timeTotal_sd<-gridMonth$timeTotal_sd/mean(totalTime$totHrs)
...
write.csv(gridMonth, file=output_file1, row.names=FALSE)