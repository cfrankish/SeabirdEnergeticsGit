...
energyRes_day<-allResults[grep("energyDay", allResults)]
...
# Try a first match #
matching<-energyRes_day[grepl(ids[k], energyRes_day)]

if (length(matching)<1) {

# Remove character matching problems
ids2<-gsub("ø", "o", ids)
energyRes_day2<-gsub("ø", "o", energyRes_day)
idx <- which(grepl(ids2[k], energyRes_day2, fixed = TRUE))

# subset to bird k
birdSub<-fread(energyRes_day2[idx])
