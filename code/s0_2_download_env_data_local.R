### This script downloads env data to match with positions of tracked birds ###
### All data is from Copernicus: reanalysis-era5-single-levels-monthly-means ###
...
### Step s1_2_1: Determine dates for which we need environmental data ####

print("Step s1_2_1: determining dates for which we need data ...")
...
act.data.sub<-fread(act.data[i], select=c("date_time"))
        
# Summarize all existing dates
act.data.sub[, `:=`(date = as.IDate(date_time), date_time = NULL)]
alldates<-data.frame(date=unique(act.data.sub$date)) 
...
#### Step s1_2_2: Download time-matching monthly environmental data ####

#### SST/SEA-ICE/AIR TEMP ####
...
file3 <- wf_request(
  request  = request1,  # the request
  transfer = TRUE,     # download the file
  path     = airtempTarget       # store data in current working directory
)
