...
### Step s1_1_0: Set-up a for loop for every species ####

# Determine species list
speciesList<-c("Black-legged kittiwake", "Northern fulmar", "Atlantic puffin",
               "Common guillemot", "Brünnich's guillemot", "Little auk")

# Determine location of input data
wetdry_files<-list.files("data/wetdry_database/", full.names=TRUE)

# Open metadata information
info<-readRDS(wetdry_files[grep("metadata", wetdry_files)])

# Begin for loop
for (i in 1:length(speciesList)) {

# Print status update
print(paste0("Step s1_1_0: looping through species ", i, "/", length(speciesList)))  
  
# subset to species i:     
input_species<-speciesList[i]
...
speciesSub<-subset(possibleSpecies, speciesNormal==input_species)
...
### Step s1_1_1: load wet-dry data ###

print("Step s1_1_1: loading wetdry data...")

# Download wet-dry data from species of interest				
wetdry_recording<-readRDS(wetdry_files[grep(input_species, wetdry_files)])

# Subset relevant columns of the metadata file
infoMeta<-info %>%
...
### Step s1_1_2: Match it with IRMA data (by session_id) ###

print("Step s1_2_2: Matching with IRMA data...")
...
idsNoMatch<-rbind(idsNoMatch1, idsNoMatch2)
...
### Step s1_1_3: Assign sampling interval & remove those that don't fit ###

print("Step s1-1_3: Remove tracks with rare sampling interval...")
...
# Step s1_1_4: Standardize to between 0 & 1 by dividing by max conductivity reading

print("Step s1_1_4: Standardizing immersion data to 0-1...")
...
### Step s1_1_5: Remove juveniles ###

print("Step s1_1_5: Removing juveniles...")
...
### Step s1_1_6: Save outputs ###

print("Step s1_1_6: Saving outputs...") 
...
# Save final product
write.csv(act.final, file = paste0("data/wetdry_raw/", speciesNameSave, "_wetdry_", today, ".csv"), row.names = FALSE) # wet-dry data by species

print("Done")

} # end of species-specific loop
